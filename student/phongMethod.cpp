...
void phong_FS(OutFragment&outFragment,InFragment const&inFragment,Uniforms const&uniforms){
  ...
  /// \image html images/fragment_shader_tasks.svg "Vizualizace výpočtu ve fragment shaderu" width=1000
    ...
    float y = inFragment.attributes[0].v4[1];
    
    float offset = sin(y * 10) / 10;
    float x = inFragment.attributes[0].v4[0] + offset;
    ...
    glm::vec3 camera = uniforms.uniform[3].v3 - inFragment.attributes[0].v3;


    glm::vec3 light = uniforms.uniform[2].v3 - inFragment.attributes[0].v3;
    
    
    glm::vec3 normal = inFragment.attributes[1].v3;

    float difuse = ((normal.x * light.x) + (normal.y * light.y) + (normal.z * light.z));
